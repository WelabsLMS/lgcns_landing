...
export default function ApplyForm() {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if(formData.applyRoute.length === 0) {
            alert('지원경로를 1개 이상 선택해주세요.');
            return;
        }
        if(formData.availableLanguages.length === 0) {
            alert('활용 가능한 프로그래밍 언어를 1개 이상 선택해주세요.');
            return;
        }
    }
                    ...
                    <div>
                        ...
                        <div className="w-full bg-white rounded-lg shadow-lg">
                            <div className="flex justify-between items-center p-4 cursor-pointer" id="accordion-header" onClick={() => toggleAccordion()}>
                                <Input type="checkbox" id="agree" title="동의합니다!" name="agreeCheck" checked={formData.agreeCheck} onChange={handleChange} required />
