...
function SelectionEle({ title }: { title: string }) {
    return (
        <div className='relative rounded-full flex w-min h-min items-center justify-center mb-8 md:mb-0'>
            <div className='rounded-full bg-[#E1EAFF] w-14 h-14 md:w-40 md:h-40 blur-[10px] md:blur-[15px]' />
            <p className='absolute text-sm md:text-3xl font-semibold text-nowrap'>{title}</p>
        </div>
    )
}
